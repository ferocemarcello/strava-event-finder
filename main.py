...
from geo import club_query, geocode, haversine
...
async def events(
    ...
    # Geocode all unique city+country combinations concurrently
    unique_queries: dict[str, tuple[float, float] | None] = {}
    geocode_tasks = {}
    for club in clubs:
        q = club_query(club)
        if q and q not in geocode_tasks:
